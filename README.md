...
For this project the aerospike server is run on a docker container. For installing Aerospike refer [here.](https://www.aerospike.com/docs/operations/install/)
For quickly testing out the project by running Aerospike server in a docker container follow the below steps: 

<b> Steps for testing out the project in Aerospike instance running in a docker container :</b>
...
2) ```com.Test.Aerospike``` package has the driver classes. ```App.java``` and ```AppAsnyc.java``` are the driver classes in this packages which executes the project Synchronously and Asynchronously.
3) ```com.Test.Aerospike.Service``` packages has the logic which executes the processes in both ways.
4) Driver classes has the methods commented out. Uncomment what you need and comment what you don't.
4) Run the project.
